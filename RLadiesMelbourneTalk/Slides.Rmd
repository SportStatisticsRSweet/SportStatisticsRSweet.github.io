...
Image: [Moneyball]()]
???
* The benefits of using data and analytics to recruit and develop players was explored in the 2003 book by Michael Lewis, later adapted into a film starring the one and only Brad Pitt. If you haven't read the book or seen the film, Moneyball is a nonfictional story of how Billy Beane, a general manager with Oakland A used sabermetrics to recruit players that were undervalued, in order to compete with the more wealthier teams in the baseball league. Beane's assistant (fictional) GM Peter Brand (famously played by Jonah Hill) is a young Yale economics graduate with radical ideas about how to assess player value. Peter Brand is supposedly based upon a collection of people, although the character is certainly about Paul DePodesta, an economics grad from Harvard.
...
???
* Data forms a large part of sport research. For example, we use data to measure and monitor performance during a drill, training session, week of training, match and a competitive season/ pre-season period. This is thanks to the adoption of new technologies by sporting organisations combined with greater financial resources is producing data at record rates. Faster, cheaper and increased access to this data means that the manner in which decisions can now be made compared to previously is vastly different.
* However, these drivers have also created new challenges such as how to handle incompatible data formatting, understanding the increased complexity of applied environments, as well as developing methods to integrate humans and machines in decision-making processes.
* I'll now take you through some different types of data we capture at the WB, as part of this strategic research partnership, to help improve on-field performance.
...
???
* Monitoring how an athlete responds to a training stimulus, such as a weights session or a training session with the team, is an important data source for sport scientists. Termed internal load, measuring the perceived load of training helps give more context. For example, all athletes on a team may complete the same training session, running for the same total time 60mins. Although the external load is the samee, each athlete may perceive how "hard" the session is differently. By tracking how this percevied rating is over time, we can start to gain more context of how an athlete is responding to and subsequently, ready for the next session. Work out of the VU-WB partnership, viewable by clicking the hyperlink here, used linear and non-linear analysis to predict this rating of perceived exertion, by athletes. It was found that independent of analysis approach, distance covered by an athlete during a training session or match is most strongly associated with RPE.
--
<br>
...
???
* Wearable sensors, such as accelerometers, can measure acceleration in three dimensions. This data source can also be used to estimate energy expenditure during training and matches, by providing a practical, non-invasive and effective method to do so. For example, by knowing how much energy an athlete expends during training and matches, dietitians are able to develop more individualised nutrition programming to enhance performance and achieve body composition goals.
...
???
* Scores are exciting, as they tell us the live or end result.
---
class: left, top
...
* So if we take a kick, as an example, we can assess the influence that these different constraints have together, on the likelihood of an effective kick. We can do this through rule mining techniques, specifically in R, the arules package.
...
* Bounded rationality as a theoretical framework  to understand  processes  of  decision-making  in  applied  sport  science.    The  nature  of  these decisions will continue to change as data quality and volume increases.
